...
管理员可在 `/admin/settings` 修改组织名称并填写原因，服务端按组织和版本更新，审计写入与修改处于同一事务。页面只展示服务是否配置，不把配置存在视为连接验证成功，也不展示存储凭证。业务时区、语言与截止时间沿用当前业务规则，不通过此页面修改。启用此版本前须执行 `0014_organization_settings` 迁移，为组织信息增加并发版本字段。

老板端 `/activity` 展示同组织且仍有可见关联资源的业务变更，管理员完整审计入口仍为 `/admin/audit`。业务入口不展示账号安全、批量导出伪资源、已删除且无法关联的资源或他人的草稿保存事件；报告提交后也不会公开先前草稿事件。仅任务转交展示操作原因，其他审计原始原因保留在管理员入口。数据库集成门禁包含该查询的组织隔离、角色隔离与草稿提交前后可见性检查；本地尚无 PostgreSQL，待 CI 或独立测试数据库执行确认。
