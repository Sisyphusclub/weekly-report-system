...
export interface ComboChartCardProps {
  ...
  tiles?: boolean;
  /**
   * Which series the big number reads from. Defaults to the bars.
   *
   * Switch it to `line` when the line is the headline metric and the bars are
   * the supporting count: traffic against conversions, say, where the reader
   * wants the traffic figure large and the conversions beside it.
   */
  headlineFrom?: "bar" | "line";
  /**
   * Replaces the header caption. Receives the hovered row, or `undefined` at
   * rest, so a caption can name several of a row's fields rather than only the
   * one series the card would otherwise show.
   */
...
export function ComboChartCard({
  ...
  const lineTotal = data.reduce((sum, row) => sum + Number(row[line.key] ?? 0), 0);
  const headlineSeries = fromLine ? line : bar;
  const headlineValue = hovering
    ? valueAt(headlineSeries.key, activeIndex)
    : (headline ?? (fromLine ? lineTotal : barTotal));
  // While hovering, the other series' value for that category rides along as
  // the caption - the two axes only make sense read together.
  const otherSeries = fromLine ? bar : line;
  const otherFormat = (fromLine ? barFormat : lineFormat) as (n: number) => string;
  const captionValue = hovering
    ? valueAt(otherSeries.key, activeIndex)
    : fromLine
      ? barTotal
      : lineAverage;
      ...
      {tiles && (
        <ChartStatTiles
          items={[
            ...
            {
              // Always the line's own figure. `captionValue` is whichever
              // series the header is *not* showing, so with headlineFrom="line"
              // it is the bar total, and formatting that as a rate printed
              // numbers like 420700%.
              label: `${line.label} · ${hovering ? "this month" : "average"}`,
              value: lineFormat(hovering ? valueAt(line.key, activeIndex) : lineAverage),
