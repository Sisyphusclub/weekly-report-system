...
/** Column spans from `sm` up, on the six-column grid (3 per row, remainder
 *  stretched). Written as literal classes so Tailwind can see them. */
const TILE_SM_SPAN: Record<number, string> = {
  2: "sm:col-span-2",
  3: "sm:col-span-3",
  6: "sm:col-span-6",
};
...
export function ChartStatTiles({
  ...
  // Phones get two per row instead of three: at 390px a third of the card is
  // too narrow for a swatch plus a label, so every name truncated to "Vis…".
  // An odd count stretches its last tile across, same as the sm+ remainder.
  const mobileOrphan = items.length % 2 === 1;
  return (
    <div className={cx("-mx-2 -mb-1 grid grid-cols-2 gap-2 sm:grid-cols-6", className)}>
      {items.map((item, i) => {
        ...
        return (
          <div
            ...
            className={cx(
              ...
              mobileOrphan && i === items.length - 1 ? "col-span-2" : "col-span-1",
              TILE_SM_SPAN[span],
            )}
            style={{ opacity: hovering && !active ? 0.4 : 1 }}
